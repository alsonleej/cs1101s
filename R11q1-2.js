//R11 q1-2
//file:///C:/Users/alson/Downloads/reflection-R11.pdf
...
function evaluate(component, env) {
    return is_literal(component)
           ...
           : is_logical_composition(component) //q2
           ? eval_logical_composition(component, env)
...
function list_of_values(exps, env) {
    return reverse(map(arg => evaluate(arg, env), exps)); //1
}
...
function eval_logical_composition(comp, env) {
    if (logical_symbol(comp) === "&&"){
        return evaluate(make_conditional_expression(
            logical_composition_first_component(comp),
            logical_composition_second_component(comp),
            make_literal(false)), env);
    } else { // its ||
        return evaluate(make_conditional_expression(
            logical_composition_first_component(comp),
            make_literal(true),
            logical_composition_second_component(comp)), env);
    }
}
