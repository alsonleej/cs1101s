...
function sequence_statements(stmt) { //QUESTION 1
    
   let seq =  head(tail(stmt));
    function hoist(stmt) {
        let seq = null;
        function helper(stmt){
            if (is_null(stmt)){
                return null;
            }
            else if (head(head(stmt)) === "function_declaration"){
                seq = pair(head(stmt) , seq);
                return helper(tail(stmt));
            }
            else {
                return pair(head(stmt), helper(tail(stmt)));
            }
        }
        const res = helper(stmt);
